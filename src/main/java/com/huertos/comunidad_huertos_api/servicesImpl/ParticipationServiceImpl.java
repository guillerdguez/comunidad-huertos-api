    ...
    public ParticipationResponseDTO updateParticipation(UUID id, ParticipationRequestDTO participationRequest) {
        ...
        Event event = eventRepository.findById(participationRequest.getEventId())
                .orElseThrow(() -> new EventNotFoundException(
                        "Event not found with ID: " + participationRequest.getEventId()));
        participation.setEvent(event);

        User user = userRepository.findById(participationRequest.getUserId())
                .orElseThrow(() -> new UserNotFoundException(
                        "User not found with ID: " + participationRequest.getUserId()));
